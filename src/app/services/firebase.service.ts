...
export class FirebaseService {
  ...
  constructor(private afs: AngularFirestore) {
    this.noteCollection = this.afs.collection<Note>('durians');
  ...
  updateNote(note: Note): Promise<void> {
    return this.noteCollection.doc(note.id).update({ name: note.name, time: note.time, tel: note.tel, address: note.address , img: note.img , link: note.link, lat: note.lat, lng: note.lng});  }
